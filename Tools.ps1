...
$installDir = "$env:USERPROFILE\Downloads\VoidTools"
...
<Window
    ...
    <Window.Resources>
        ...
        <SolidColorBrush x:Key="SidebarBg"  Color="#12121F"/>
        <SolidColorBrush x:Key="CardBg"     Color="#1A1A2E"/>
        <SolidColorBrush x:Key="Accent"     Color="#00F0FF"/>
        <SolidColorBrush x:Key="AccentDim"  Color="#0088AA"/>
        <SolidColorBrush x:Key="TextMain"   Color="#E0F8FF"/>
        <SolidColorBrush x:Key="TextMuted"  Color="#667788"/>
        <SolidColorBrush x:Key="ConsoleBg"  Color="#05050F"/>
                    ...
                    <ControlTemplate TargetType="Button">
                        ...
                        <ControlTemplate.Triggers>
                            <Trigger Property="IsMouseOver" Value="True">
                                <Setter Property="Background" Value="#1F1F33"/>
                            </Trigger>
                        </ControlTemplate.Triggers>
                    </ControlTemplate>
                </Setter.Value>
    ...
    <Border Background="{StaticResource MainBg}" BorderBrush="#1F2A44" BorderThickness="1" CornerRadius="8">
        <Grid>
            <Grid.RowDefinitions>
            ...
            <Border Grid.Row="0" Background="{StaticResource SidebarBg}" CornerRadius="8,8,0,0">
                <Grid Margin="16,0">
                    ...
                    <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
                        ...
                        <TextBlock Text="  VoidTools" FontSize="14" FontWeight="SemiBold" Foreground="{StaticResource TextMain}"/>
                        <TextBlock Text="  -  by acousticvoid" FontSize="11" Foreground="{StaticResource TextMuted}" VerticalAlignment="Center" Margin="4,0,0,0"/>
                    </StackPanel>
            ...
            <Grid Grid.Row="1">
                ...
                <!-- Sidebar -->
                <Border Grid.Column="0" Background="{StaticResource SidebarBg}" BorderBrush="#1F2A44" BorderThickness="0,0,1,0">
                    <StackPanel Margin="10,14,10,14">

                        <Border Background="#0C0C1A" CornerRadius="6" Margin="0,0,0,14" Padding="0,10">
                        ...
                        <Separator Background="#1F2A44" Margin="0,10,0,10"/>

                        <TextBlock Text="CREDITS" FontSize="9" FontWeight="Bold" Foreground="{StaticResource TextMuted}" Margin="4,0,0,6"/>
                        <TextBlock Text="Made by acousticvoid" FontSize="11" FontWeight="SemiBold" Foreground="{StaticResource TextMain}" Margin="4,2,0,4"/>
                        <TextBlock Text="Discord: acousticvoid" FontSize="10" Foreground="{StaticResource TextMuted}" TextWrapping="Wrap" Margin="4,1,0,0"/>
                        <TextBlock Text="GitHub: acousticvoid" FontSize="10" Foreground="{StaticResource TextMuted}" TextWrapping="Wrap" Margin="4,1,0,0"/>

                        <Separator Background="#1F2A44" Margin="0,10,0,10"/>
                        <TextBlock x:Name="InstPathBlock" Text="" FontSize="9" Foreground="#445577" TextWrapping="Wrap" Margin="4,0"/>
                    </StackPanel>
                </Border>
                                            ...
                                            <ControlTemplate TargetType="TabItem">
                                                ...
                                                <ControlTemplate.Triggers>
                                                    ...
                                                    <MultiTrigger>
                                                        ...
                                                        <Setter TargetName="TabBorder" Property="Background" Value="#1F1F33"/>
                                                        <Setter Property="Foreground" Value="{StaticResource TextMain}"/>
                                                    </MultiTrigger>
                                                </ControlTemplate.Triggers>
                                            </ControlTemplate>
...
<Window
    ...
    Background="Transparent"
    FontFamily="Segoe UI">
    <Border Background="#0A0A12" BorderBrush="#1F2A44" BorderThickness="1" CornerRadius="8" Padding="24">
        <Grid>
            <Grid.RowDefinitions>
                <RowDefinition Height="*"/>
            ...
            <StackPanel Grid.Row="0">
                <TextBlock Text="VoidTools" FontSize="20" FontWeight="Bold" Foreground="#00F0FF" Margin="0,0,0,12"/>
            ...
            <Grid Grid.Row="1" VerticalAlignment="Bottom">
                ...
                <Button x:Name="CancelBtn" Grid.Column="0" Content="Cancel" Height="40"
                        Background="Transparent" Foreground="#E0F8FF" BorderBrush="#1F2A44" BorderThickness="1"
                        Cursor="Hand" FontSize="13"/>
                <Button x:Name="AcceptBtn" Grid.Column="2" Content="Accept &amp; Continue" Height="40"
                        Background="#1A1A2E" Foreground="#00F0FF" BorderBrush="#00F0FF" BorderThickness="1"
...
function Start-CmdToolCommand {
    ...
    $tempScript = [System.IO.Path]::Combine($env:TEMP, "void_$([guid]::NewGuid().ToString('N')).ps1")
    Set-Content -LiteralPath $tempScript -Value $Command -Encoding UTF8 -Force

    $startArgs = '/c start "VoidTools" powershell.exe -NoExit -NoProfile -ExecutionPolicy Bypass -File "' + $tempScript + '"'
...
function Save-UrlToFile {
    ...
    $client.Headers.Add("User-Agent", "VoidTools")
    try {
...
function Get-GitHubAssetUrl {
    ...
    if ($ReleaseUrl -match "github\.com/([^/]+)/([^/]+)/releases/tag/(.+)$") {
        ...
        try {
            $rel   = Invoke-RestMethod -Uri $api -Headers @{"User-Agent"="VoidTools"} -ErrorAction Stop
...
function Start-ButtonAnimation {
    ...
    # Flash sequence: cyan -> white -> cyan
    $flashColors = @("#00F0FF", "#FFFFFF", "#00F0FF", "#88FFFF")
    $flashFg     = "#0A0A12"
...
foreach ($cat in $Categories) {
    ...
    foreach ($tool in $catTools) {
        ...
        switch ($t.Type) {
            "Cmd"    { $btn.Background = "#1A1A2E" }
            "GitHub" { $btn.Background = "#1A1A2E" }
            "Web"    { $btn.Background = "#1A1A2E" }
            "Link"   { $btn.Background = "#1A1A2E" }
        }

        $btnBg    = [Windows.Media.SolidColorBrush]::new([Windows.Media.Color]::FromRgb(0x1A, 0x1A, 0x2E))
        $btnScale = [Windows.Media.ScaleTransform]::new(1.0, 1.0)
        $btnGlow  = [Windows.Media.Effects.DropShadowEffect]::new()
        $btnGlow.Color       = [Windows.Media.Color]::FromRgb(0x00, 0xF0, 0xFF)
        ...
        $btn.Add_MouseEnter({
            ...
            $a  = [Windows.Media.Animation.ColorAnimation]::new([Windows.Media.Color]::FromRgb(0x00,0xF0,0xFF), $d)
        ...
        $btn.Add_MouseLeave({
            ...
            $ease = [Windows.Media.Animation.CubicEase]::new()
            $a  = [Windows.Media.Animation.ColorAnimation]::new([Windows.Media.Color]::FromRgb(0x1A,0x1A,0x2E), $d)
            $bg.BeginAnimation([Windows.Media.SolidColorBrush]::ColorProperty, $a)
            ...
            $b.Foreground = [Windows.Media.BrushConverter]::new().ConvertFrom("#E0F8FF")
        })
        ...
        $btn.Add_Click({
            ...
            $script:timer.Add_Tick({
                ...
                if ($sc2) {
                    ...
                    if ($bg2) {
                        $aC = [Windows.Media.Animation.ColorAnimation]::new([Windows.Media.Color]::FromRgb(0x1A,0x1A,0x2E), $dRel)
                        $bg2.BeginAnimation([Windows.Media.SolidColorBrush]::ColorProperty, $aC)
                    }
                }
                $script:timerBtn.Foreground = [Windows.Media.BrushConverter]::new().ConvertFrom("#E0F8FF")
...
# VOID ANIMATION (simple pulsing void symbol)
...
$script:voidIdx = 0
$voidTimer = New-Object System.Windows.Threading.DispatcherTimer
$voidTimer.Interval = [TimeSpan]::FromMilliseconds(1200)
$voidTimer.Add_Tick({
    $script:voidIdx = ($script:voidIdx + 1) % $voidFrames.Count
    $CatBlock.Text = $voidFrames[$script:voidIdx]
})
$voidTimer.Start()
...
$CloseBtn.Add_Click({ $voidTimer.Stop(); $window.Close() })
